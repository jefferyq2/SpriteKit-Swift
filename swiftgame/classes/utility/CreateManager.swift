...
struct CreateManager {
    ...
    static func createBackground(parent : SKScene)->(background : SKSpriteNode, parallax : [ParallaxSprite]){
        ...
        background.position = CGPoint(x: parent.frame.midX, y: parent.frame.midY)
    ...
    static func createCharacter(parent : SKScene)->SKSpriteNode{
        ...
        let characterFrames = [f1,f2,f3,f4,f5,f6,f7,f8,f9,f10,f11,f12,f13,f14]
        
        let player : SKSpriteNode = SKSpriteNode(imageNamed: "character_01.png")
        player.position =  CGPoint(x: parent.frame.midX - 50, y: parent.frame.midY - 150)
        
        parent.addChild(player)
        
        let action : SKAction = SKAction.repeatForever(SKAction.animate(with: characterFrames, timePerFrame: 0.1))
        player.run(action)
